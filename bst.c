...
struct bst{
  ...
  void (*free)(void *); //frees the generic values stored in tree
  int (*compare)(void *,void *);  //compares the generic void * value
  void (*swap)(BSTNODE *,BSTNODE *);  //swaps the node values
};

BST *newBST(void (*d)(FILE *,void *),void (*f)(void *),int (*c)(void *,void *),void (*s)(BSTNODE *,BSTNODE *)){
...
void displayBST(FILE *fp,BST *t){
  ...
  BSTNODE *x;
  QUEUE *buffer = newQUEUE(t->display,t->free);
...
void freeBST(BST *items){
	BSTNODE *x;
	while(sizeBST(items) != 0){
		x = getBSTroot(items);
		x = swapToLeafBST(items,x);
		pruneLeafBST(items,x);
		items->free(getBSTNODE(x));
		free(x);
	}
	free(items);
}
